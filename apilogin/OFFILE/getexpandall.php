...
function getExpandAll()
{
    try {
        ...
        $response = getDataChildren($searchapidata);
...
function groupItems($parent, $items)
{
    $children = array();
    foreach ($items as $item) {
        if ($item['ParentId'] == $parent['id']) {
            $child = groupItems($item, $items);
            $children[] = $child;
        }
    }
...
function getDataChildren($searchapidata)
{
    ...
    try {
        ...
        // Lấy các id của searchData không có key children
        $searchapidataFilter = array_filter($searchapidata, 'hasNoChildren');
        $searchDataIds = array_column($searchapidataFilter, 'id');
        if (!empty($searchDataIds) > 0) {
            $stmt = sqlGetDataChildren($searchDataIds);
            $response = assignNewDataChildren($stmt, $searchapidata);
        } else {
