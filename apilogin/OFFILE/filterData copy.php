...
function sqlGetData($valueSearch, $valueColumn, $currentPage, $rowsPerPage,)
{
    ...
    try {
        ...
        $sql = "SELECT *
        FROM (
            SELECT DENSE_RANK() OVER (ORDER BY devices." . COLUMN_Devices_NAME . ", COALESCE(inventories." . COLUMN_INVENTORIES_PARENTID .
        ...
        WHERE device_rank BETWEEN $start and $end";
        ...
        $sqlDevicesId = "SELECT distinct devices." . COLUMN_Devices_ID . " as device_id,devices." . COLUMN_Devices_NAME . " as device_name
            FROM " . TABLE_DEVICES . " devices 
            INNER JOIN " . TABLE_INVENTORIES . " inventories 
            ON devices." . COLUMN_Devices_ID . " = inventories." . COLUMN_INVENTORIES_PARENTID . "
            WHERE " . COLUMN_STATUS . " <> 'D' AND $where1 AND $where2 ";
        $stmt = $conn->prepare($sqlDevicesId);
        foreach ($params as $key => $value) {
            $stmt->bindValue($key, $value);
        }
        $stmt->execute();
        ...
        return array("inventories" => $inventories, "devices" =>  $idNameFilter, "total_devices" => sizeof($devices));
    } catch (Error $th) {
